    ...
    class Program
    {
        ...
        private static string Extract(string s, string s1)
        {
            ...
            if (val < 0 || val > s.Length)
                return null;
            return t=s.Substring(s.Length-val,val);
        }
        private static void Print(string s, string s1)
        {
            ...
            if (s1 != "")
            {
                t = Extract(s, s1);
                if (t!=null)
                    Console.WriteLine("Из строки {0} справа выделена подстрока {1}", s, t);
                else
        ...
        static void Main(string[] args)
        {
            ...
            for (; ; )
            {
                ...
                Console.Write("Введите размер подстроки выделяемой справа: ");
                s1=Console.ReadLine();
                Print(s, s1);
            }
