...
function Poisk(X: Integer; MAS: TMas): Integer;
 ...
 var
  M: Integer;
 begin
  if Rg<=Lg then begin y:=y+1; Result:=Lg; Exit; end;
  M:=(Lg+Rg) div 2;
  if MAS[M]=X then begin y:=y+1; Result := M; Exit; end;
  y:=y+1;
  if MAS[M]>X then
...
SMas(mas1);
Vyvod(mas1);
if Poisk(C,mas1)=C then Writeln(Poisk(C,mas1)) else Writeln('Otveta net');
Writeln('Sravnenij=',y);
