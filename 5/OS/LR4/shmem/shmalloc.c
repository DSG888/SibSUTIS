 ...
 */
void *_shmalloc(int id, size_t *size, void *shmptr, size_t shm_size, char *filename, int linenumber)
{
	...
	if (*size == 0)
	{
		...
		fprintf(stderr, "%s, line %d: Warning: allocating a pointer of size zero returns NULL.\n", filename, linenumber);
		return NULL;
	}
	if (*size < 0)
	{
		fprintf(stderr, "%s, line %d: Cannot allocate a negative amount of memory in shmalloc().\n", filename, linenumber);
		return NULL;
	}
	if (shm_size < *size + sizeof(Header))
	{
		fprintf(stderr, "%s, line %d: Insufficient memory to fulfill the memory allocation request.\n", filename, linenumber);
		return NULL;
	}
	...
	else
	{
		...
		if(best_fit == NULL)
		{
			...
			fprintf(stderr, "%s, line %d: shmalloc() ran out of available space to satisfy the request.\n", filename, linenumber);
			pthread_mutex_unlock(&(first->mutex));
			return NULL;
		}
...
void _shmfree(void *shmptr, size_t shm_size, void *shm_ptr, char *filename, int linenumber)
{
	...
	if(h->bitseq != BITSEQ)
	{
		fprintf(stderr, "%s, line %d: Attempted to free a pointer not allocated by shmalloc() or corruption of internal memory has occurred. Check your memory accesses.\n", filename, linenumber);
		return;
	}
	if (h->is_free)
	{
		fprintf(stderr, "%s, line %d: Attempt to shmfree() a pointer that has already been freed.\n", filename, linenumber);
		return;
	}
