...
begin
	fp := yp2 + cos(yp2 + 1) + 2 * y - sin(2 * x) + yp1;
end;
...
procedure f(x:real;y1:vector;var fxy:vector);
...
begin
	...
	repeat     {MPD}
		c:=(b+a)/2;
		if (fp(a,y1[1],y1[0],x)*fp(c,y1[1],y1[0],x)<0) 
		then b:=c
		else a:=c;
	until abs(a-b)<0.1;
	y2:=(a+b)/2;        {y2 - y с точностью 0.1}
	
	repeat  {Метод Hьютона}
		y:=y2;
...
procedure rung(x:real;y:vector;h:real;var yo:vector);
...
procedure dvoinoy_pereschet(x0,xn:real;yn:vector;var rezy:vector);
var	x	: real;
	kp	: boolean;
	y,y1,y2: vector;
begin
...
begin
	...
	yk[0]:=y[0];
	yk[1]:=a;
	dvoinoy_pereschet(0,1,yk,yr);
	writeln(' k=',k:5:7,'  y=',yr[0]:5:7,' [',a:2:2,';',b:2:2,']');
	...
	repeat
		...
		yk[1]:=k;
		dvoinoy_pereschet(0,1,yk,yr);
		writeln(' k=',k:5:7,'  y=',yr[0]:5:7,' [',k-h:2:2,';',k:2:2,'] - интервал для k');
	...
	repeat     {MPD}
		...
		dvoinoy_pereschet(0,1,yk,yr);
		yk[1]:=c;
		dvoinoy_pereschet(0,1,yk,yr1);
		if (yr[0]-y[1])*(yr1[0]-y[1])<0 
		then b:=c
		else a:=c;
		writeln(' k=',((a+b)/2):5:7,'  y=',yr[0]:5:7);
	until abs(a-b)<e;

	kzad:=(a+b)/2;{Параметр K}
end;
...
BEGIN
	...
	for i := 1 to 9 do 
	begin
		dvoinoy_pereschet(0,masx[i],ynach,yr);
		masy[i] := yr[0];
	...
	for i := 1 to n do  
	begin
		dvoinoy_pereschet(0,x,ynach,yr);
		mas1x[i] := x;
		mas1y[i] := yr[0];
		x := x + 0.2;
	end;
