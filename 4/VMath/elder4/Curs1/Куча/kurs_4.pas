...
Function DP(kr:real):real;
Var
  h,x,y,y1,y_2,y1_2,yy:real;
begin
  h:=0.1;
  x:=0;
  y:=1;
...
x:=0;
i:=0;
while (x-Endh)<=1 do begin
  if round(x*100000)=round(0.2*i*100000) then begin
    writeln('y(',x:2:2,')= ',y:8:5,'   y''(',x:2:2,')= ',y1:8:5);
    y0[i]:=y;
    inc(i);
  end;
  MRK(Endh,x,y,y1);
