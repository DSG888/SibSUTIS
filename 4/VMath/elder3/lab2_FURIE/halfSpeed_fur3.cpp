...
int main()
{
 ...
 A1[0]=(float**)malloc(sizeof(float*)*p1); // действительная
 A1[1]=(float**)malloc(sizeof(float*)*p2);                  // мнимая  

 A2=(float***)malloc(sizeof(float**)*2);
 
 A2[0]=(float**)malloc(sizeof(float*)*p1); // действительная
 A2[1]=(float**)malloc(sizeof(float*)*p2);                  // мнимая  
 ...
 for(k1=0; k1 < p1; k1++)
 {
  for(k2=0; k2 < p2; k2++)
  {
   ...
   for(j2=0; j2 < p2; j2++)
   {
    ...
    A2[0][k1][k2]+=A1[0][k1][j2]*cos(arg)-A1[1][k1][j2]*sin(arg);
    A2[1][k1][k2]+=A1[0][k1][j2]*cos(arg)+A1[1][k1][j2]*sin(arg);   
   }          
   A2[0][k1][k2]/=p1;
   A2[1][k1][k2]/=p2;       
  }
